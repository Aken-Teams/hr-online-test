...
| 步驟 | 名稱 | 內容 |
|------|------|------|
| 1 | 基本信息 | 標題、考試時長、理論/實操權重、合格分數、基本題比例、**梯次設置（可選）** |
...
| 考試梯次 | 可選 | 將考試分為多個時間窗口，梯次時間須在考試範圍內 |
...
| 考試管理 | 考試列表（含篩選）、多步驟精靈建立考試（含梯次設置）、Tabs 詳情頁（基本信息/人員/成績/規則）、發佈、監控、刪除 |
...
| 儀表板 | 歡迎訊息、系統介紹、快捷入口 |
| 我的考試 | 四組下拉篩選（考試名稱/工序/級別/狀態）+ 分頁瀏覽（9張/頁），預設顯示進行中考試 |
| 考試須知 | 查看考試規則與注意事項 |
...
| 項目 | 調整內容 | 原因 |
|------|---------|------|
| 考試梯次 | 新增梯次管理（ExamBatch），支援將考試分為多個時間窗口 | 因空間限制需分批考試，梯次卡控考試時間窗口 |
| 我的考試篩選 | 改為四組 CustomSelect 下拉篩選（考試/工序/級別/狀態）+ 分頁 | 改善考生在多考試場景下的瀏覽體驗 |
| 考試列表篩選 | 管理後台考試列表新增狀態、標題篩選 | 考試數量增多時快速定位 |
| 批次匯入優化 | 題庫與人員匯入改為 createMany 批次寫入 | 大幅提升匯入效能 |
| AI 欄位識別 | Excel 匯入新增 DeepSeek AI 自動識別欄位 | 相容不同格式的 Excel 檔案 |
| 已封存考試 | 考生端我的考試與成績查詢可見已封存考試 | 考生需查看歷史考試記錄 |
