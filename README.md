...
- **我的考試** — 多維度篩選（考試名稱 / 工序 / 級別 / 狀態），分頁瀏覽
- **線上考試** — 選擇題 + 判斷題，全程計時，到時自動交卷，梯次時間窗口卡控
...
- **題庫管理** — 選擇題 / 判斷題分類管理，支援 Excel 批次匯入（批次 createMany 高效寫入）
- **考試管理** — 靈活組卷、設定考試時間與評分規則、梯次（分時段）管理、發布/關閉考試
...
專案附帶 `data/` 目錄，包含完整的部門題庫資料：

| 分類 | 部門數 | 題目數量 | 說明 |
|------|--------|---------|------|
| 基本知識題庫 | — | 246 題 | 通用安全/5S/基礎知識 |
| 部門專業題庫 | 6 個部門 | ~5,028 題 | 按工序+級別分類 |
| **合計** | — | **~5,274 題（去重後約 4,531 題）** | — |

涵蓋部門：生產部、工務部、制程品管部、工程研發部、資材部、客戶質量部。

可透過管理後台的「考試管理 → 建立考試 → 匯入題庫」功能批次匯入，系統自動從檔名解析工序/級別/分類。
