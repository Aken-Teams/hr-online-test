    ...
    <div className="flex min-h-screen flex-col items-center justify-center px-4 py-12">
      ...
      <div className="mb-10 grid w-full max-w-3xl grid-cols-1 gap-4 sm:grid-cols-3">
        {INFO_CARDS.map((card) => (
          <div
            key={card.title}
            className="flex flex-col items-center rounded-xl border border-gray-200 bg-white px-6 py-6 text-center shadow-sm"
          >
            <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-indigo-50">
              {card.icon}
            </div>
            <h3 className="text-sm font-semibold text-gray-900">{card.title}</h3>
            <p className="mt-1 text-lg font-bold text-indigo-600">{card.value}</p>
