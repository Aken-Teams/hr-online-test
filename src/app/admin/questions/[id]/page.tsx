...
import type { QuestionType, QuestionData } from '@/types/exam';
...
export default function EditQuestionPage() {
  ...
  const fetchQuestion = useCallback(async () => {
    try {
      ...
      const q: QuestionData = json.data;
      ...
      if (q.type === 'SINGLE_CHOICE' || q.type === 'MULTI_CHOICE') {
        if (q.correctAnswer) {
          setCorrectOptions(new Set(q.correctAnswer.split(',')));
        }
      } else if (q.type === 'TRUE_FALSE') {
        setTfAnswer(q.correctAnswer === '是' ? 'true' : 'false');
      } else if (q.type === 'FILL_BLANK') {
        setFillAnswer(q.correctAnswer || '');
      }

      // SA fields
      if (q.referenceAnswer) setReferenceAnswer(q.referenceAnswer);
      if (q.gradingRubric) setGradingRubric(q.gradingRubric);
    } catch {
      toast('加载题目失败', 'error');
    } finally {
      setLoading(false);
