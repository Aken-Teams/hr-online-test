...
| Excel 批次匯入 | ✅ 完成 | 支援 .xls / .xlsx 格式，createMany 批次寫入 |
| AI 欄位識別 | ✅ 完成 | DeepSeek AI 自動識別 Excel 欄位對應 |
...
| 梯次管理 | ✅ 完成 | 將考試分為多個時間窗口（梯次），梯次時間須在考試範圍內 |
| 考試列表篩選 | ✅ 完成 | 管理後台考試列表支援狀態/標題篩選 |
...
| 功能 | 狀態 | 說明 |
|------|------|------|
| Excel 批次匯入 | ✅ 完成 | 工號、姓名、部門、崗位、身份證後 6 位（createMany 優化） |
...
| 功能 | 狀態 | 說明 |
|------|------|------|
| 我的考試篩選 | ✅ 完成 | 四組 CustomSelect 下拉篩選（考試/工序/級別/狀態） |
| 分頁瀏覽 | ✅ 完成 | 每頁 9 張卡片，支援分頁導覽 |
| 預設篩選 | ✅ 完成 | 預設選擇最新活躍考試，狀態預設為進行中 |
| 考試梯次 | ✅ 完成 | ExamBatch 模型，支援多時間窗口卡控 |
| 梯次時間驗證 | ✅ 完成 | 梯次時間必須在考試開放~截止範圍內 |
| 梯次時間判斷 | ✅ 完成 | isInExamTimeWindow() 梯次感知，向後相容 |
